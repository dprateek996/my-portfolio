    ...
    <main className={`min-h-screen bg-zinc-950 text-zinc-200 selection:bg-white/20 ${inter.className}`}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-20 relative z-10">
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-24">
          ...
          {/* Projects (Grid) with Z-Index Fixes */}
          <div className="col-span-1 md:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-4 mt-4" id="projects">
            {PROJECTS.map((project, i) => (
              <SpotlightCard key={i} className="p-6 group relative overflow-hidden flex flex-col h-full">

                {/* 1. Gradient Blob (Background) */}
                <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${project.gradient} blur-[60px] rounded-full opacity-0 group-hover:opacity-40 transition-opacity duration-500 pointer-events-none`} />

                {/* 2. Diagram Overlay (Background Layer) */}
                <div className="absolute inset-0 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center bg-zinc-950/95 backdrop-blur-[2px] pointer-events-none">
                ...
                <a
                  href={project.link}
                  target="_blank"
                  className="absolute bottom-6 left-6 right-6 py-2 bg-zinc-100 text-zinc-950 text-center font-bold rounded-lg opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 z-30"
                >
                  View Details
                </a>
          ...
          <div className="col-span-1 md:col-span-12 mt-12 mb-12">
            <div className="flex items-center justify-between mb-6">
              ...
              <a
                ...
                target="_blank"
                className="text-sm font-mono text-emerald-400 hover:text-emerald-300 flex items-center gap-2 transition-colors"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {EXTRA_PROJECTS.map((project, i) => (
                <motion.a
                  ...
                  className="group relative p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-xl hover:bg-zinc-800/60 hover:border-zinc-700 transition-all duration-300 flex flex-col h-full"
                >
